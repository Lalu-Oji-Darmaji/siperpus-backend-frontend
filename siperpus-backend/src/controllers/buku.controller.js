...
// GET /api/buku — Ambil semua buku dengan filter dan pagination
...
async function create(req, res, next) {
    try {
        const { judul, penulis, penerbit, tahun, isbn,
            kategori, sinopsis, stok } = req.body
        const [result] = await pool.execute(
            ...
            [judul, penulis, penerbit, tahun, isbn,
                kategori, sinopsis, stok, stok] // tersedia = stok awal
        )
        const [newBuku] = await pool.execute(
            'SELECT * FROM buku WHERE id = ?', [result.insertId]
        )
        res.status(201).json({
            success: true,
        ...
        if (err.code === 'ER_DUP_ENTRY') {
            return next(createError(409, 'ISBN sudah digunakan'))
        }
        next(err)
    }
}
...
async function update(req, res, next) {
    try {
        ...
        await pool.execute(
            ...
            [judul, penulis, penerbit, tahun, isbn,
                kategori, sinopsis, stok, req.params.id]
        )
        const [updated] = await pool.execute(
            'SELECT * FROM buku WHERE id = ?', [req.params.id]
        )
        res.json({
            success: true, message: 'Buku berhasil diperbarui', data:
                updated[0]
        })
    } catch (err) {
        next(err)
...
async function remove(req, res, next) {
    try {
        const [existing] = await pool.execute(
            'SELECT id FROM buku WHERE id = ?', [req.params.id]
        )
        if (existing.length === 0) throw createError(404, 'Buku tidak ditemukan')
...
// GET /api/buku/statistik — untuk dashboard
