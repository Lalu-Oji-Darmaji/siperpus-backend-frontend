// backend/src/controllers/peminjaman.controller.js (ringkasan)
...
VALUES (?, ?, ?)`,
        ...
        res.status(201).json({
            success: true,
            message: `Buku '${buku.judul}' berhasil dipinjam`, data: { peminjamanId: result.insertId, tanggalKembali }
        })
    } catch (err) {
        await conn.rollback()
        conn.release()
...
async function kembalikanBuku(req, res, next) {
    ...
    try {
        ...
        const terlambat = Math.max(0, Math.ceil((today - batas) /
            (1000 * 60 * 60 * 24)))
        ...
        res.json({
            ...
            data: { terlambatHari: terlambat, denda }
        })
    } catch (err) {
        await conn.rollback()
        conn.release()
...
module.exports = { pinjamBuku, kembalikanBuku /* ...getAll, getRiwayat */ }